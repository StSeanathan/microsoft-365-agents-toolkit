    ...
    it("Provision Resource: workflow bot", async function () {
      const cmd =
        runtime === Runtime.Node
          ? `atk new --interactive false --app-name ${appName} --capability workflow-bot --programming-language typescript`
          : `atk new --interactive false --runtime ${runtime} --app-name ${appName} --capability workflow-bot`;
      await execAsync(cmd, {
        cwd: testFolder,
        env: env,
        timeout: 0,
